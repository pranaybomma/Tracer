...
def _SelectSrc():
	...
	All_Ec2s=_Ec2s(All_VPCs[_Slct][0])
...
def _SgEgress(Src,Dst,protocol):
...
def _SgIngress(Src,Dst,protocol,port):
						...
						if (ip_address(Src[0]) in ip_network(_cidr['CidrIp'])):
							if 'FromPort' in _IRules:
								if _IRules['FromPort'] != _IRules['ToPort']:
									if port in range(_IRules['FromPort'],_IRules['ToPort']):
										_IngressCount += 1
...
def _SNetAcl(Src,Dst,protocol,port):
	''' Network Acl check 
		 In              Out
		 1024-65535		port
		 Des            Des
					...
					if (ip_address(Dst[0]) in ip_network(_NAcls['CidrBlock'])) and ((str(protocol) == _NAcls['Protocol']) or (_NAcls['Protocol'] == '-1')):
						if 'PortRange' in _NAcls:
							if _NAcls['PortRange']['From'] == 1024 and _NAcls['PortRange']['To']== 65535:
								...
								print(f"\n{'Ingress NACL:'}\n{'-'*32}\n{'RuleNumber:'}{_NAcls['RuleNumber']}{'Deny due to Ephemeral ports'}\n{'-'*32}")
								_InCount += 1
						else:
...
def _Route(Src,Dst):
	...
	Src.append(ec2.Subnet(id=Src[1]).vpc_id)
	Dst.append(ec2.Subnet(id=Dst[1]).vpc_id)
	Src.append(ec2.Vpc(id=Src[3]).owner_id)
	Dst.append(ec2.Vpc(id=Dst[3]).owner_id)
	Src.append(ec2.Vpc(id=Src[3]).cidr_block)
	Dst.append(ec2.Vpc(id=Dst[3]).cidr_block)	
...
def _DNetAcl(Src,Dst,protocol,port):
	''' Network Acl check 
		 In              Out
		 port		1024-65535
		 src            src
...
def main():
	protocol = input('Please Enter the Protocol: ').lower()
	if protocol == 'tcp':
		protocol_num = 6
	if protocol == 'udp':
		protocol_num = 17
	...
	_SelectDst()
	if Src[0]!=Dst[0]:
		if ec2.Subnet(Src[1]).vpc_id == ec2.Subnet(Dst[1]).vpc_id:
			if ec2.Subnet(Src[1]) == ec2.Subnet(Dst[1]):
				_SgEgress(Src,Dst,protocol)
				_SgIngress(Src,Dst,protocol,port)
				print()
				print('Src and Destination are located in same VPC and same SUBNET')
			else:
				_SgEgress(Src,Dst,protocol)
				print()
				_SNetAcl(Src,Dst,protocol_num,port)
				_Route(Src,Dst)
				_DNetAcl(Src,Dst,protocol_num,port)
				_SgIngress(Src,Dst,protocol,port)
